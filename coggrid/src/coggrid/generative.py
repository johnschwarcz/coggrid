...
.. code-block:: text
    ...
    orthonormal_embeddings   ->  K, Q                 (n_states, n_obs, dim)
    ...
    marginal_likelihood      ->  factorised rates     (n_eps, n_obs, n_ctx, R)
...
def sigmoid(x: np.ndarray) -> np.ndarray:
    ...
    The original used ``1 / (1 + np.exp(-x))``, which emits an overflow warning
    and produces ``inf`` intermediates for strongly negative ``x``. This branch
    is mathematically identical and quiet.
    """
...
def orthonormal_embeddings(
    ...
    """Random unit vectors, orthogonalised across observation channels.

    Returns an array of shape ``(n_states, n_observations, embedding_dim)`` in
    which, for every state ``s``, the ``n_observations`` vectors
    ``out[s, :, :]`` form an orthonormal set.

    Orthogonality across channels is what makes the observation channels carry
    *independent* information about the same pair of latent states: the
    ...
    v = rng.standard_normal((n_states, n_observations, embedding_dim))
    for o in range(n_observations):
        # Modified Gram-Schmidt against the already-normalised earlier channels.
        for prev in range(o):
...
def value_profile(cfg: CogGridConfig) -> np.ndarray:
    ...
    the shift is what makes each realisation of a latent state prefer a
    ...
    instead of piling mass up at the edges of the realisation axis.
    """
...
def realization_potential(
    ...
    """Map interaction strengths to a potential over realisations.
    ...
    a normalised weight vector, which is then dotted against each rolled row of
    the profile. Smooth, periodic, and monotone in ``|z|`` locally — which is
    what lets nearby latent states produce nearby observation statistics.
    """
...
def interaction_strength(
    ...
    """Inner product between the key of active state ``i`` and the query of ``j``.
    ...
    which is what lets a pair of states produce an anisotropic joint
...
def joint_likelihood(
    ...
    """Bernoulli rates for every joint realisation of the active states.
    ...
    keys, queries:
        ``(n_states, n_observations, embedding_dim)`` embeddings.
    ctx_inds:
        ``(n_episodes, n_contexts)`` indices of the active latent states.
    profile:
    ...
    rates:
        ...
        with ``n_contexts`` trailing realisation axes. ``rates[b, o, r0, r1]``
        is ``P(observation o == 1)`` in episode ``b`` when active state 0 takes
        value ``r0`` and active state 1 takes value ``r1``.
    interactions:
        ``(n_episodes, n_observations, n_interactions)`` raw strengths, in the
        order ``(0,1), (1,0), (0,2), (2,0), ...``. Useful for analysis; not
        needed to run the environment.
    ...
    *probability* is not, and neither is the resulting observation
    distribution. That non-factorisation is the thing this environment exists
    to measure.
    """
    if profile is None:
    ...
    if n_contexts == 1:
        # Degenerate case: a state interacts with itself, giving a single
        # potential over the one realisation axis.
        ...
        for i in range(n_contexts):
            for j in range(i + 1, n_contexts):
                ...
                # Outer product over the two realisation axes, then broadcast
                # into the full (n_ctx)-dimensional realisation grid by giving
...
def marginal_likelihood(rates: np.ndarray, n_contexts: int) -> np.ndarray:
    """Collapse the joint rates to one independent table per active state.

    Returns ``(n_episodes, n_observations, n_contexts, n_realizations)``, where
    entry ``[b, o, c, r]`` averages the joint rate over all realisations of
    every context *other* than ``c``.

    This is the likelihood available to an observer that has (wrongly) assumed
    the active states are independent. Averaging — rather than maximising or
    conditioning — corresponds to marginalising under a uniform prior over the
    other states' values, which is exactly the prior an agent starts an episode
    with.
    """
...
def _draw(
    ...
    """Draw ``(n_episodes, n_contexts)`` states from ``pool``."""
    if not distinct:
        return rng.choice(pool, size=(n_episodes, n_contexts))
    if n_contexts > pool.size:
        raise ValueError(
            f"cannot draw {n_contexts} distinct states from a pool of {pool.size}"
        )
    # Vectorised per-row sampling without replacement.
...
def sample_contexts(
    ...
    """Choose which latent states are active, and which one carries the goal.

    Parameters
    ----------
    split:
        ``"held_out"``
            Every active state comes from the held-out pool
            (``range(n_held_out_states)``). Nothing in the episode was seen
            during training — the hard generalisation condition.
        ``"train"``
            States come from the whole pool, but **at most one** held-out state
            is allowed per episode and the goal is never a held-out state. This
            is the compositional condition: a novel state appears alongside
            familiar ones, and the agent is scored on a familiar one.

    Returns
    -------
    ctx_inds:
        ``(n_episodes, n_contexts)`` sorted indices of the active states.
    goal_ind:
    ...
    second state to pair a novel one with, so the "at most one held-out state"
    rule is vacuous and the single state (hence the goal) may be held out.
    """
    n_episodes = cfg.n_episodes if n_episodes is None else n_episodes
    all_states = np.arange(cfg.n_states)
    n_held_out = int(cfg.n_held_out_states or 0)
    held_out, train = all_states[:n_held_out], all_states[n_held_out:]

    if cfg.subsample_states is not None:
        held_out = rng.choice(held_out, size=cfg.subsample_states, replace=False)
        train = rng.choice(train, size=cfg.subsample_states, replace=False)

    distinct = not cfg.allow_repeated_states
    ...
    pool = all_states if cfg.subsample_states is None else np.concatenate([train, held_out])
    ctx = _draw(pool, n_episodes, cfg.n_contexts, distinct, rng)

    # Keep at most one held-out state per episode; replace any extras.
    ...
    # The goal must be a familiar state: zero out held-out columns before argmax.
...
def sample_realizations(
    ...
    """Draw a value for each active state, uniformly over realisations.
...
def observation_rates(rates: np.ndarray, ctx_vals: np.ndarray) -> np.ndarray:
    """Pick out the true Bernoulli rate for the realisation that actually occurred.
...
def sample_observations(
    ...
    realisations to rates, not in tracking a moving target.
    """
